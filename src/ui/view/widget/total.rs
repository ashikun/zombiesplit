...
impl Widget {
    fn render_total(&self, r: &mut dyn Renderer, s: &state::Footer) -> gfx::Result<()> {
    ...
    fn render_at_cursor(&self, r: &mut dyn Renderer, s: &state::Footer) -> gfx::Result<()> {
        render_label(r, "Up to cursor")?;
        // TODO(@MattWindsor91): positioning hack here.
        r.set_font(font::Id::Large);
        r.set_pos(self.rect.point(
            0,
            r.font_metrics()[font::Id::Large].span_h(1),
            Anchor::TOP_RIGHT,
        ));
...
fn render_label(r: &mut dyn Renderer, label: &str) -> gfx::Result<()> {
    r.set_font(font::Id::Medium);
