...
pub fn total_type(t: &event::total::Type) -> Result<session::event::Total> {
    Ok(match t {
        event::total::Type::Attempt(delta) => attempt_total_type(delta)?,
        event::total::Type::Comparison(ty) => comparison_total_type(*ty)?,
    })
}

fn attempt_total_type(delta: &super::super::RunDelta) -> Result<session::event::Total> {
    super::timing::run_delta(delta).map(session::event::Total::Attempt)
}
...
fn split(s: &event::Split) -> Result<session::Event> {
    ...
    let event = match Missing::SplitEvent.require(s.payload.as_ref())? {
        ...
        event::split::Payload::Delta(p) => split_delta(p)?,
...
fn split_delta(delta: &super::super::SplitDelta) -> Result<session::event::Split> {
    Ok(session::event::Split::Delta(super::timing::split_delta(
        delta,
    )?))
}
